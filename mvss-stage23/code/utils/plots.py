...
def plot(model, indices, model_outputs ,pose, rgb_gt, path, epoch, img_res, plot_nimgs, max_depth, resolution):
    ...
    spec_s0 = model_outputs['s0_spec']
    diff_s0 = model_outputs['s0_diff']
    ...
    plot_images(rgb_eval, rgb_gt, spec_s0, diff_s0, path, epoch, plot_nimgs, img_res)

    # 绘制透明置信图看看
    plot_transparent_confidence(model_outputs['transparent_confidence'], path, epoch, plot_nimgs, img_res)

    # 绘制融合aolp置信图看看
    plot_fused_aolp(model_outputs['fused_aolp'][:,0], path, epoch, plot_nimgs, img_res)
    ...
    network_object_maskk = network_object_mask[0]
    normals_cam[:, network_object_maskk, ] = rend_util.get_normals_cam(normals, pose)[:, network_object_maskk, :]
...
def plot_images(rgb_points, ground_true, spec_s0, diff_s0, path, epoch, plot_nrow, img_res):
    ground_true = (ground_true.cuda() + 1.) / 2.
    rgb_points = (rgb_points + 1. ) / 2.
    spec_s0 = torch.clip(spec_s0, 0.0, 1.0).unsqueeze(0).unsqueeze(2).repeat(1,1,3)
    diff_s0 = torch.clip(diff_s0, 0.0, 1.0).unsqueeze(0).unsqueeze(2).repeat(1,1,3)

    output_vs_gt = torch.cat((spec_s0, diff_s0, rgb_points, ground_true), dim=0)
...
def plot_transparent_confidence(transparent_confidence, path, epoch, plot_nrow, img_res):
    tensor = lin2img(transparent_confidence.unsqueeze(0).unsqueeze(2), img_res)[0,0].cpu().numpy()

    # tensor = transparent_confidence.transpose(1, 2, 0)
    scale_factor = 190
    tensor = 255 - (tensor * scale_factor).astype(np.uint8)

    img = Image.fromarray(tensor)
    img.save('{0}/trans_conf_{1}.png'.format(path, epoch))


def plot_fused_aolp(fused_aolp, path, epoch, plot_nrow, img_res):
    tensor = lin2img(fused_aolp.unsqueeze(0).unsqueeze(2), img_res)[0, 0].cpu().numpy()
    tensor = (tensor*255 / np.pi).astype(np.uint8)

    img = Image.fromarray(tensor)
    img.save('{0}/fused_aolp_{1}.png'.format(path, epoch))
