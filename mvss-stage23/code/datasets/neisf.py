    ...
    def __init__(self,
        ...
        if data_category == 'Pandora':
            ...
            for i in range(self.n_images):
                s0 = rend_util.load_rgb_npy(f'{self.instance_dir}/s0/{i:04d}.npy', 1)
                ...
                # img = Image.fromarray((s0 * 255).astype(np.uint8))
                # img.show()
            ...
            # for i in range(self.n_images):
                ...
                self.s0_ori.append(s0.transpose(2,0,1))
                self.s1_ori.append(s1.transpose(2,0,1))
                self.s2_ori.append(s2.transpose(2,0,1))
            ...
            for i in range(self.n_images):
                image_name = "00"+str(i) if i<10 else "0" + str(i)
                image_name = "100" if image_name == "0100" else image_name
                s0 = rend_util.load_exr(f'{self.instance_dir}/images_s0/img_{image_name}.exr', 1) / 255.0
                s1 = rend_util.load_exr(f'{self.instance_dir}/images_s1/img_{image_name}.exr', 1) / 255.0
                s2 = rend_util.load_exr(f'{self.instance_dir}/images_s2/img_{image_name}.exr', 1) / 255.0
        ...
        for path_idx in range(len(dolp_paths)):
            ...
            dolpp = dolp.clone()
            dolpp[torch.from_numpy(self.s0_ori[path_idx] < 1e-6)] = 0.0
            # import cv2
            # im = cv2.cvtColor(aolp.numpy(), cv2.COLOR_BGR2RGB)
            # cv2.imshow("k", im[:,:,1]/np.pi)
            # cv2.waitKey(0)
            self.origin_dolp.append(dolpp.float())
            dolpp = dolpp.reshape(3,-1).transpose(1,0)  # (0,1)
            # aolp = aolp * np.pi # (0,pi)
            self.dolp_images.append(dolpp.float())
