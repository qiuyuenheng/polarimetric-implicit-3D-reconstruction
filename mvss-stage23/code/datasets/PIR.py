    ...
    def __init__(self,
        ...
        if data_category == 'Pandora':
            ...
            for i in range(1,self.n_images + 1):
                ...
                s0 = 0.5 * rend_util.load_hdr(f'{self.instance_dir}/images_stokes/{int(image_name):02d}_s0.hdr', downscale)
                s0p1 = 0.5 * rend_util.load_hdr(f'{self.instance_dir}/images_stokes/{int(image_name):02d}_s0p1.hdr', downscale)
                s0p2 = 0.5 * rend_util.load_hdr(f'{self.instance_dir}/images_stokes/{int(image_name):02d}_s0p2.hdr', downscale)
                s1 = s0p1 - s0
                s2 = s0p2 - s0
        ...
        if data_category == 'NeISF':
            ...
            for i in range(1,self.n_images + 1):
                ...
                # if s0.max() > m0: m0 = s0.max()
                # if s1.max() > m1: m1 = s1.max()
                # if s2.max() > m2: m2 = s2.max()
                #
                # from PIL import Image
                # im = Image.fromarray(np.uint8(s0))
                # img = Image.fromarray(np.clip(np.uint8(s0 * 255), 0, 255))
                # img.save("kkk0.png")
                # img = Image.fromarray(np.clip(np.uint8(s1*s0*255),0, 255))
                # img.save("kkk1.png")
                # img = Image.fromarray(np.clip(np.uint8(s2*s0 * 255), 0, 255))
                # img.save("kkk2.png")

                self.s0.append(s0.reshape(-1))
                self.s1.append(s1.reshape(-1))
                self.s2.append(s2.reshape(-1))
        ...
        # note:each stokes vector shape:(512*612, 3)
        self.object_masks = []
        ...
        for path in aolp_paths:
            ...
            # aolp = np.zeros((3, 700, 700)).astype(np.float32)
            self.origin_aolp.append(torch.from_numpy(aolp).float())
        ...
        for path in dolp_paths:
            ...
            # dolp = np.zeros((3, 700, 700)).astype(np.float32)
            self.origin_dolp.append(torch.from_numpy(dolp).float())
