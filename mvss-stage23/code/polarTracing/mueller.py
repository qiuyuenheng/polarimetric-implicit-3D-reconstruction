...
class Mueller(nn.Module):
    ...
    #     eta = eta.reshape(-1)
    #     cos_theta_t= self.fresnel_equation(cos_theta_i,eta)
    #
    #     cos_theta_t = torch.abs(cos_theta_t)
    #     sin_theta_i, sin_theta_t = torch.sqrt(1 - cos_theta_i ** 2 + 1e-8), torch.sqrt(1 - cos_theta_t ** 2 + 1e-8)  # sin xi,sin xt
    #     cos_theta_i_subtract_t = cos_theta_i*cos_theta_t + sin_theta_i*sin_theta_t  # cos(xi-xt)
    #     cos_theta_i_plus_t = cos_theta_i*cos_theta_t - sin_theta_i*sin_theta_t  # cos(xi+xt)
    #     sin_theta_i_plus_t = sin_theta_i * cos_theta_t + cos_theta_i * sin_theta_t  # sin(xi+xt)
    #     sin_theta_i_subtract_t = sin_theta_i * cos_theta_t - cos_theta_i * sin_theta_t  # sin(xi-xt)
    #     tan_theta_i_subtract_t = sin_theta_i_subtract_t / (cos_theta_i_subtract_t + 1e-6)  # tan(xi-xt)
    #     factor = 0.5 * tan_theta_i_subtract_t * tan_theta_i_subtract_t / (sin_theta_i_plus_t * sin_theta_i_plus_t + 1e-6)
    #
    #     a = factor*(cos_theta_i_subtract_t * cos_theta_i_subtract_t + cos_theta_i_plus_t * cos_theta_i_plus_t)
    #     b = factor*(cos_theta_i_subtract_t * cos_theta_i_subtract_t - cos_theta_i_plus_t * cos_theta_i_plus_t)
    #     c = -2 * cos_theta_i_subtract_t * cos_theta_i_plus_t * factor
    #
    ...
    def specular_reflection(self,cos_theta_i,eta):
        ...
        cos_theta_i = cos_theta_i.reshape(-1)
        eta = eta.reshape(-1)
        # cos_theta_i = torch.clamp(cos_theta_i, min=0.0, max=1.0)  # 我加的
    ...
    #     n_rays = cos_theta_i.shape[0]
    #
    #     # a_s：sin α-/α+, a_p：tan α-/α+, cos_theta_t：cos(xt), eta_it, eta_ti
    #     cos_theta_t = self.fresnel_equation(cos_theta_i, eta)
    #
    #     cos_theta_t = torch.abs(cos_theta_t)
    #     cos_theta_t = torch.clamp(cos_theta_t, min=0.0, max=1.0)
    #     sin_theta_i, sin_theta_t = torch.sqrt(1 - cos_theta_i**2 + 1e-8), torch.sqrt(1 - cos_theta_t**2 + 1e-8)
    #     cos_theta_i_subtract_t = cos_theta_i*cos_theta_t + sin_theta_i*sin_theta_t  # cos(xi-xt)
    #     sin_theta_i_plus_t = sin_theta_i*cos_theta_t + cos_theta_i*sin_theta_t # sin(xi+xt)
    #     factor = 0.5 * (4*sin_theta_i*cos_theta_i*sin_theta_t*cos_theta_t) / (sin_theta_i_plus_t*sin_theta_i_plus_t*cos_theta_i_subtract_t*cos_theta_i_subtract_t +1e-7)
    #
    #     a = factor * (cos_theta_i_subtract_t * cos_theta_i_subtract_t + 1)
    #     b = factor * (cos_theta_i_subtract_t * cos_theta_i_subtract_t - 1)
    #     c = -2 * cos_theta_i_subtract_t * factor
    #
    #     res = torch.zeros([n_rays,4,4]).cuda().float()
    #     res[:,0,0] = a
    #     res[:,1,1] = a
    ...
    def specular_transmission(self,cos_theta_i,eta):
        ...
        c = factor * torch.sqrt(t_p * t_s + 1e-6)
    ...
    def frenel_polarized(self,cos_theta_i, eta):
        ...
        # cos_theta_i_abs = torch.clamp(cos_theta_i_abs, min=0.0, max=1.0)
        # cos_theta_t_sqr = torch.clamp(cos_theta_t_sqr, min=0.0, max=1.0)
    ...
    def fresnel_equation(self,cos_theta_i, eta):
        # eta: n2/n1
        # cos_theta_i: (n_rays,) float
        # eta: (n_rays) float
        # return: cos_theta_t_signed: float
        cos_theta_i = cos_theta_i.reshape(-1)
        eta = eta.reshape(-1)
        rcp_eta = 1.0 / eta
        outside_mask = cos_theta_i >= 0.0
        ...
        cos_theta_t = torch.sqrt(cos_theta_t_sqr + 1e-6)
